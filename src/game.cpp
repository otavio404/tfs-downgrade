...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.primary.value > 0) {
		...
		if (realHealthChange > 0 && !target->isInGhostMode()) {
			...
			int div = 0;
			if (realHealthChange >= 1000) {
				div = realHealthChange % 1000;
				if (div > 0) {
					strHealthChange << realHealthChange / 1000 << "." << div << "k";
				}
				else {
					strHealthChange << realHealthChange / 1000 << "k";
				}
			}
			else {
		...
		if (targetPlayer && target->hasCondition(CONDITION_MANASHIELD) && damage.primary.type != COMBAT_UNDEFINEDDAMAGE) {
			...
			if (manaDamage != 0) {
				...
				if (manaDamage >= 1000) {
					div = manaDamage % 1000;
					if (div > 0) {
						strManaDamage << manaDamage / 1000 << "." << div << "k";
					}
					else {
						strManaDamage << manaDamage / 1000 << "k";
					}
				}
				else {
		...
		if (message.primary.value) {
			...
			if (message.primary.color != TEXTCOLOR_NONE) {
				...
				int div = 0;
				if (damage.primary.value >= 1000) {
					message.primary.value = damage.primary.value / 1000;
					div = damage.primary.value % 1000;
					if (div > 0) {
						strPrimaryDamage << message.primary.value << "." << div << "k";
					}
					else {
						strPrimaryDamage << message.primary.value << "k";
					}
				}
				else {
		...
		if (message.secondary.value) {
			...
			if (message.secondary.color != TEXTCOLOR_NONE) {
				...
				if (damage.secondary.value >= 1000) {
					message.secondary.value = damage.secondary.value / 1000;
					strSecondaryDamage << message.secondary.value << "k";
				}
				else {
